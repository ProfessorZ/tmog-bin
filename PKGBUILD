...
depends=('glibc' 'hicolor-icon-theme' 'libgcc' 'libstdc++' 'qt6-base' 'qt6-multimedia'
         'qt6-svg' 'systemd-libs')
...
_srcroot() {
  ...
  if (( ${#_dirs[@]} != 1 )) || [[ ! -d ${_dirs[0]} ]]; then
    printf 'expected exactly one top-level directory in the upstream tarball\n' >&2
    return 1
  fi
  printf '%s' "${_dirs[0]%/}"
}
